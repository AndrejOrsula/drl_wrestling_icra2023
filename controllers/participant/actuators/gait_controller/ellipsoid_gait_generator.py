...
import numpy as np
from sensors.pose_estimator import PoseEstimator


class EllipsoidGaitGenerator:
    # TODO: Tweak gait parameters
    ...
    def __init__(self, robot, time_step, imu):
        ...
        self.lateral_leg_offset = (
            0.05  # y distance between the center of mass and one foot
        )
        self.step_period = 0.4  # time to complete one step
        # amplitudes of stride:
        self.step_length_front = (
            self.MAX_STEP_LENGTH_FRONT
        )  # when heading in the front direction (x axis)
        self.step_length_side = (
            self.MAX_STEP_LENGTH_SIDE
    ...
    def compute_leg_position(self, is_left, desired_radius=1e3, heading_angle=0):
        ...
        factor = (
            -1 if is_left else 1
        )  # the math is the same for both legs, except for some signs
        ...
        if abs(desired_radius) > 0.1:
            ...
            amplitude_x = (
                self.in_place_step_length
                * (turning_radius * rotate_right - factor * self.lateral_leg_offset)
                / turning_radius
                * rotate_right
            )
            x = factor * amplitude_x * np.cos(self.theta)
    ...
    def compute_z(self, is_left):
        ...
        amplitude_z += (
            factor
            * self.pose_estimator.get_roll_pitch_yaw()[0]
            * self.roll_reflex_factor
        )
        # extensor response: pushes on the leg when it is on the ground
        force_values = (
            self.left_foot_sensor.getValues()
            if is_left
            else self.right_foot_sensor.getValues()
        )
    ...
    def rotate(x, y, angle):
        ...
        return x * np.cos(angle) - y * np.sin(angle), x * np.sin(angle) + y * np.cos(
            angle
        )
