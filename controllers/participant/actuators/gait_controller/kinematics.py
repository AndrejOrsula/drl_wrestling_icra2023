...
import constants.kinematics as constants
import numpy as np
...
class Kinematics:
    ...
    def reset(self):
        ...
        self.left_leg_previous_joints = [0, 0, 0, 0, 0, 0]
        self.right_leg_previous_joints = [0, 0, 0, 0, 0, 0]
    ...
    def forward_left_leg(cls, thetas):
        ...
        T_base_end = (
            A_base_0 @ T_0_1 @ T_1_2 @ T_2_3 @ T_3_4 @ T_4_5 @ T_5_6 @ Rot_zy @ A_6_end
        )
        return np.concatenate(
            (T_base_end[0:3, 3], cls.transform_to_orientation(T_base_end))
        )
    ...
    def inverse_leg(self, x, y, z, roll, pitch, yaw, is_left):
        ...
        theta_4_double_prime = np.pi - np.arccos(
            (constants.ThighLength**2 + constants.TibiaLength**2 - d**2)
            / (2 * constants.ThighLength * constants.TibiaLength)
        )
        for theta_4_test in [theta_4_double_prime, -theta_4_double_prime]:
            if joint_limits.LKneePitchLow < theta_4_test < joint_limits.LKneePitchHigh:
                solution_tree_root.add_child_node(theta_4_test)
        T_tilde_prime = T_tilde @ np.linalg.inv(
            self.get_T_5_6(theta_6) @ self.get_Rot_zy()
        )
        T_double_prime = np.linalg.inv(T_tilde_prime)
        for theta_4_node in solution_tree_root.children:
            theta_4_test = theta_4_node.angle
            numerator = T_double_prime[1, 3] * (
                constants.TibiaLength + constants.ThighLength * np.cos(theta_4_test)
            ) + constants.ThighLength * T_double_prime[0, 3] * np.sin(theta_4_test)
            denominator = (
                constants.ThighLength**2 * np.sin(theta_4_test) ** 2
                + (constants.TibiaLength + constants.ThighLength * np.cos(theta_4_test))
                ** 2
            )
            ...
            for theta_5_test in [
                theta_5_prime,
                (np.pi if theta_5_prime >= 0 else -np.pi) - theta_5_prime,
            ]:
                if (
                    joint_limits.LAnklePitchLow
                    < theta_5_test
                    < joint_limits.LAnklePitchHigh
                ):
        ...
        for theta_4_node in solution_tree_root.children:
            for theta_5_node in theta_4_node.children:
                ...
                for theta_2_test in [
                    theta_2_prime - plus_or_minus_pi_over_4,
                    -theta_2_prime - plus_or_minus_pi_over_4,
                ]:
                    if (
                        joint_limits.LHipRollLow
                        < theta_2_test
                        < joint_limits.LHipRollHigh
                    ):
                    ...
                    theta_3_prime = np.arcsin(
                        T_triple_prime[1, 1]
                        / np.sin(theta_2_test + plus_or_minus_pi_over_4)
                    )
                    theta_3 = []
                    for theta_3_test in [
                        theta_3_prime,
                        (np.pi if theta_3_prime >= 0 else -np.pi) - theta_3_prime,
                    ]:
                        if (
                            joint_limits.LHipPitchLow
                            < theta_3_test
                            < joint_limits.LHipPitchHigh
                        ):
                    ...
                    theta_1 = []
                    for theta_1_test in [
                        theta_1_prime + np.pi / 2,
                        -theta_1_prime + np.pi / 2,
                    ]:
                        if (
                            joint_limits.LHipYawPitchLow
                            < theta_1_test
                            < joint_limits.LHipYawPitchHigh
                        ):
        ...
        if len(combinations) == 0 or len(combinations[0]) != 6:
            ...
            best_solution = (
                self.left_leg_previous_joints
                if is_left
                else self.right_leg_previous_joints
            )
