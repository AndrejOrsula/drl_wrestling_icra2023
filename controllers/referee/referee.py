...
TRAIN = False
DISPLAY_INFO = not TRAIN

SIMULATION_MODE = Supervisor.SIMULATION_MODE_REAL_TIME
...
class Referee(Supervisor):
    def init(self):
        ...
        self.game_duration = 3 * 60 * 1000  # a game lasts 3 minutes
        self.time_step = int(self.getBasicTimeStep())

    def display_time(self, minutes, seconds):
        for j in range(3):
            self.digit[j][self.current_digit[j]].setPosition(
                1000
    ...
    def run(self, CI, display_info: bool = True):
        ...
        if display_info:
            participant = (
                os.environ["PARTICIPANT_NAME"]
                if "PARTICIPANT_NAME" in os.environ
                else "Participant"
            )
            opponent = (
                os.environ["OPPONENT_NAME"]
                if "OPPONENT_NAME" in os.environ
                else "Opponent"
            )
        ...
        while True:
            ...
            for i in range(2):
                ...
                if (
                    position[2] < 0.9
                    or position[2] > 1.5
                    or abs(position[0]) > 1.5
                    or abs(position[1]) > 1.5
                ):
                    self.ko_count[i] = self.ko_count[i] + self.time_step
                else:
                    self.ko_count[i] = 0
                if display_info:
            ...
            if (
                self.step(self.time_step) == -1
                or time > self.game_duration
                or self.ko_count[0] > 10000
                or self.ko_count[1] > 10000
            ):
                break
            time += self.time_step
        if self.ko_count[0] > 10000 and self.ko_count[0] > self.ko_count[1]:
            ...
            if self.coverage[0] > self.coverage[1]:
                print(
                    "Red wins coverage: %s > %s" % (self.coverage[0], self.coverage[1])
                )
                performance = 1
            else:  # in case of coverage equality, blue wins
                print(
                    "Blue wins coverage: %s >= %s"
        ...
        if display_info:
            self.setLabel(
                ...
                "WIN",
                0.673,
                0.051 - 0.048 * performance,
                0.08,
                0x0000FF if performance == 0 else 0xFF0000,
                0,
                "Arial",
            )
        if CI:
...
referee = Referee()
referee.simulationSetMode(SIMULATION_MODE)
